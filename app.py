    ...
    try:
        ...
        for r in res.data or []:
            ...
            utenti[codice] = {
                ...
                "ricarico_cliente": str(r.get("ricarico_cliente", "") or ""),
    ...
    for codice, d in utenti.items():
        ...
        if prof in ["RIVENDITORE", "GROSSISTA"]:
            righe.append({
                ...
                "ricarico_cliente": d.get("ricarico_cliente", ""),
            })
    return righe
        ...
        for r in reader:
            ...
            if codice:
                utenti[codice] = {
                    ...
                    "ricarico_cliente": r.get("ricarico_cliente", "")
                }
    return utenti

def salva_utente_csv(utente, password, profilo, nome, azienda, telefono, email, ricarico="35"):
    file_exists = Path(UTENTI_CSV).exists()
    campi = ["utente", "password", "profilo", "nome", "azienda", "telefono", "email", "ricarico", "ricarico_cliente", "data_creazione"]
...
def render_dashboard_crm(preventivi):
    ...
    c1.markdown(v1035_card("Preventivi", totale_preventivi), unsafe_allow_html=True)
    c2.markdown(v1035_card("Valore totale", euro(valore_totale)), unsafe_allow_html=True)
    c3.markdown(v1035_card("Utile lordo", euro(utile_totale)), unsafe_allow_html=True)
    c4.markdown(v1035_card("Accettati/Ordinati", accettati + ordinati), unsafe_allow_html=True)
    c5.markdown(v1035_card("Persi", persi), unsafe_allow_html=True)
    c6.markdown(v1035_card("Conversione", f"{conversione:.1f}%"), unsafe_allow_html=True)
...
def render_stati_preventivi(stats):
    ...
    for col, stato in zip(cols, STATI_PREVENTIVO):
        col.markdown(v1035_card(stato, stats.get(stato, 0)), unsafe_allow_html=True)
...
# =========================
# V1034 - DASHBOARD ADMIN AUTONOMA SEMPRE VISIBILE
# =========================

def v1034_float(v, default=0.0):
    try:
        if v in [None, ""]:
            v = default
        return float(str(v).replace(",", "."))
...
def v1034_ricarico_cliente_target(ricarico_acquisto, target=60):
...
def v1034_prezzo_riv(costo, ricarico_acquisto):
    return float(costo or 0) * (1 + float(ricarico_acquisto or 0) / 100)

def v1034_prezzo_cliente(costo, ricarico_acquisto, ricarico_cliente):
    return v1034_prezzo_riv(costo, ricarico_acquisto) * (1 + float(ricarico_cliente or 0) / 100)

def v1034_salva_ricarichi_csv(username, ricarico_acquisto, ricarico_cliente):
    username = str(username or "").strip().upper()
    path = Path(UTENTI_CSV)
    ...
    for r in righe:
        if str(r.get("utente", "") or "").strip().upper() == username:
    ...
    if not trovato:
        nuova = {
            ...
            "ricarico": str(ricarico_acquisto),
            "ricarico_cliente": str(ricarico_cliente),
            "data_creazione": datetime.now().strftime("%d/%m/%Y %H:%M")
        }
        for c in fieldnames:
            nuova.setdefault(c, "")
    ...
    return True, "CSV aggiornato"

def v1034_salva_ricarichi_supabase(username, ricarico_acquisto, ricarico_cliente):
    sb = supabase_client()
    if sb is None:
        return False, "Supabase non collegato"
    try:
        ...
        return True, "Supabase aggiornato"
    except Exception as e:
        return False, str(e)

def v1034_salva_ricarichi_totale(username, ricarico_acquisto, ricarico_cliente):
    msg = []
    ok_csv, msg_csv = v1034_salva_ricarichi_csv(username, ricarico_acquisto, ricarico_cliente)
    msg.append(msg_csv)
    if supabase_attivo():
        ok_sb, msg_sb = v1034_salva_ricarichi_supabase(username, ricarico_acquisto, ricarico_cliente)
        msg.append(msg_sb)
    return True, " | ".join([m for m in msg if m])
...
.v1034-admin-box {
    ...
    box-shadow:0 8px 22px rgba(6,73,155,.10)!important;
}
.v1034-title {
...
.v1034-card {
    ...
    box-shadow:0 5px 14px rgba(6,73,155,.08)!important;
}
.v1034-card-label {
    color:#06499b!important;
    -webkit-text-fill-color:#06499b!important;
...
def v1034_card(label, valore):
    return f"""
    <div class="v1034-card">
        <div class="v1034-card-label">{label}</div>
        <div class="v1034-card-value">{valore}</div>
    </div>
    """

def render_dashboard_admin_v1034(profilo):
    if profilo != "SA-TEC":
        return

    v1034_css()

    st.markdown('<div class="v1034-admin-box">', unsafe_allow_html=True)
    st.markdown('<div class="v1034-title">🔵 DASHBOARD ADMIN SA-TEC</div>', unsafe_allow_html=True)
    st.markdown('<div class="v1034-sub">Dashboard autonoma: CRM, clienti, preventivi e ricarichi rivenditori/grossisti.</div>', unsafe_allow_html=True)

    tab1, tab2, tab3 = st.tabs(["CRM", "Ricarichi Riv/Gross", "Clienti"])
    ...
    with tab1:
        ...
        c1.markdown(v1034_card("Preventivi", totale_preventivi), unsafe_allow_html=True)
        c2.markdown(v1034_card("Valore totale", euro(valore_totale)), unsafe_allow_html=True)
        c3.markdown(v1034_card("Utile lordo", euro(utile_totale)), unsafe_allow_html=True)

        cerca = st.text_input("Cerca preventivo / cliente / utente", key="v1034_cerca_prev")
        stato = st.selectbox("Filtra stato", ["Tutti"] + STATI_PREVENTIVO, key="v1034_stato_prev")
        prev_vis = filtra_preventivi_dashboard(preventivi, cerca, stato)

        st.write(f"Preventivi visualizzati: **{len(prev_vis)}**")
        if prev_vis:
            st.markdown(tabella_html_sicura(prev_vis), unsafe_allow_html=True)

            codici = [str(p.get("codice_preventivo", "") or "").strip() for p in prev_vis if str(p.get("codice_preventivo", "") or "").strip()]
            if codici:
                st.markdown("### Aggiorna stato / elimina preventivo")
                a1, a2, a3 = st.columns([2, 1, 1])
                with a1:
                    cod = st.selectbox("Preventivo", codici, key="v1034_cod_prev")
                with a2:
                    nuovo = st.selectbox("Nuovo stato", STATI_PREVENTIVO, key="v1034_nuovo_stato")
                with a3:
                    st.markdown("<br>", unsafe_allow_html=True)
                    if st.button("AGGIORNA", key="v1034_agg_prev"):
                        if aggiorna_stato_preventivo_admin(cod, nuovo):
    ...
    with tab2:
        ...
        codici = []
        for codice, d in utenti.items():
            prof_u = str(d.get("profilo", "") or "").upper()
            if prof_u in ["RIVENDITORE", "GROSSISTA"]:
                codici.append(str(codice).strip().upper())
        ...
        if not codici:
            ...
            with c1:
                ut = st.selectbox("Utente Rivenditore/Grossista", codici, key="v1034_utente_riv_gros")
                d = utenti.get(ut, {})
                prof_u = str(d.get("profilo", "") or "").upper()
                st.write(f"Profilo: **{prof_u}**")
                st.caption(f"Azienda/Nome: {d.get('azienda','') or d.get('nome','')}")
            with c2:
                ric_acq_default = v1034_float(d.get("ricarico", ricarico_default(prof_u)), ricarico_default(prof_u))
            ...
            with c3:
                ...
                if ric_cli_default in ["", None]:
                    ric_cli_default = v1034_ricarico_cliente_target(ric_acq, 60)
                ric_cli = st.number_input(
                    "Ricarico vendita cliente %",
                    min_value=0.0,
                    max_value=200.0,
                    value=float(v1034_float(ric_cli_default, 23.08)),
                    step=0.5,
                    key="v1034_ric_cli"
                )
            with c4:
                st.markdown("<br>", unsafe_allow_html=True)
                if st.button("SALVA RICARICHI", key="v1034_salva_ric"):
                    ok, msg = v1034_salva_ricarichi_totale(ut, ric_acq, ric_cli)
            ...
            p1.markdown(v1034_card("Costo SA-TEC esempio", euro(100)), unsafe_allow_html=True)
            p2.markdown(v1034_card("Prezzo Riv/Gross", euro(v1034_prezzo_riv(100, ric_acq))), unsafe_allow_html=True)
            p3.markdown(v1034_card("Prezzo Cliente Finale", euro(v1034_prezzo_cliente(100, ric_acq, ric_cli))), unsafe_allow_html=True)

            st.markdown("---")
            st.markdown("### Elimina Rivenditore / Grossista")
            st.caption("La cancellazione definitiva resta nella sezione Gestione Rivenditori/Grossisti già presente.")

    with tab3:
        clienti = carica_clienti()
        cerca_cli = st.text_input("Cerca cliente", key="v1034_cerca_cli")
        clienti_vis = filtra_clienti_dashboard(clienti, cerca_cli)
        st.write(f"Clienti visualizzati: **{len(clienti_vis)}**")
        if clienti_vis:
            st.markdown(tabella_html_sicura(righe_clienti_dashboard(clienti_vis)), unsafe_allow_html=True)
        else:
            st.info("Nessun cliente trovato.")
...
# =========================
# V1035 - CRM BLU/NERO + RICARICO VENDITA RIV/GROSS
# =========================

def v1035_float(v, default=0.0):
    try:
        if v in [None, ""]:
            v = default
        return float(str(v).replace(",", "."))
...
def v1035_ricarico_cliente_target(ricarico_acquisto, target=60):
    prezzo_riv = 100 * (1 + float(ricarico_acquisto or 0) / 100)
    prezzo_target = 100 * (1 + float(target or 0) / 100)
    if prezzo_riv <= 0:
        return 0.0
...
def v1035_prezzo_riv(costo, ricarico_acquisto):
    return float(costo or 0) * (1 + float(ricarico_acquisto or 0) / 100)

def v1035_prezzo_cliente(costo, ricarico_acquisto, ricarico_cliente):
    return v1035_prezzo_riv(costo, ricarico_acquisto) * (1 + float(ricarico_cliente or 0) / 100)

def v1035_salva_ricarichi_csv(username, ricarico_acquisto, ricarico_cliente):
    username = str(username or "").strip().upper()
    path = Path(UTENTI_CSV)
    fieldnames = ["utente", "password", "profilo", "nome", "azienda", "telefono", "email", "ricarico", "ricarico_cliente", "data_creazione"]
    righe = []
    ...
    for campo in ["ricarico", "ricarico_cliente", "data_creazione"]:
        if campo not in fieldnames:
            fieldnames.append(campo)
...
def v1035_salva_ricarichi_supabase(username, ricarico_acquisto, ricarico_cliente):
    sb = supabase_client()
    if sb is None:
        return False, "Supabase non collegato"
    try:
...
def v1035_salva_ricarichi_totale(username, ricarico_acquisto, ricarico_cliente):
    messaggi = []
    ok_csv, msg_csv = v1035_salva_ricarichi_csv(username, ricarico_acquisto, ricarico_cliente)
    messaggi.append(msg_csv)
    if supabase_attivo():
        ok_sb, msg_sb = v1035_salva_ricarichi_supabase(username, ricarico_acquisto, ricarico_cliente)
        messaggi.append(msg_sb)
    return True, " | ".join([m for m in messaggi if m])
...
.v1035-card {background:#ffffff!important;border:2px solid #06499b!important;border-radius:14px!important;padding:14px!important;min-height:86px!important;box-shadow:0 5px 14px rgba(6,73,155,.08)!important;}
.v1035-card-label {color:#06499b!important;-webkit-text-fill-color:#06499b!important;font-size:13px!important;font-weight:1000!important;text-transform:uppercase!important;}
.v1035-card-value {color:#111827!important;-webkit-text-fill-color:#111827!important;font-size:25px!important;font-weight:1000!important;margin-top:7px!important;}
</style>
""", unsafe_allow_html=True)

def v1035_card(label, valore):
    return f"""
    <div class="v1035-card">
        <div class="v1035-card-label">{label}</div>
        <div class="v1035-card-value">{valore}</div>
    </div>
    """
...
if profilo == "SA-TEC":
    ...
    st.sidebar.markdown("---")
    mostra_dashboard = st.sidebar.checkbox("Mostra dashboard SA-TEC", value=True)
    ...
    if mostra_dashboard:
        ...
        if not righe_riv:
            ...
            v1035_css_pulito()
            st.markdown("### Ricarichi Rivenditore / Grossista")
            st.caption("Ricarico acquisto = quanto compra da SA-TEC. Ricarico vendita cliente = quanto applica al cliente finale.")

            codici_riv_v1035 = [r["utente"] for r in righe_riv]
            utenti_edit_v1035 = carica_tutti_utenti()

            col_riv1, col_riv2, col_riv3, col_riv4 = st.columns([2, 1.2, 1.2, 1])
            with col_riv1:
                utente_riv_mod = st.selectbox("Utente rivenditore/grossista", codici_riv_v1035, key="utente_riv_mod_v1035")
                dati_riv = utenti_edit_v1035.get(utente_riv_mod, {})
                prof_riv = str(dati_riv.get("profilo", "") or "").upper()
                st.write(f"Profilo: **{prof_riv}**")
            with col_riv2:
                ricarico_acq_attuale = v1035_float(dati_riv.get("ricarico", ricarico_default(prof_riv)), ricarico_default(prof_riv))
                nuovo_ricarico_riv = st.number_input(
                    "Ricarico acquisto SA-TEC %",
                    min_value=0.0,
                    max_value=200.0,
                    value=float(ricarico_acq_attuale),
                    step=1.0,
                    key="nuovo_ricarico_riv_v1035"
                )
            with col_riv3:
                ricarico_cliente_attuale = dati_riv.get("ricarico_cliente", "")
                if ricarico_cliente_attuale in ["", None]:
                    ricarico_cliente_attuale = v1035_ricarico_cliente_target(nuovo_ricarico_riv, 60)
                nuovo_ricarico_cliente = st.number_input(
                    "Ricarico vendita cliente %",
                    min_value=0.0,
                    max_value=200.0,
                    value=float(v1035_float(ricarico_cliente_attuale, 23.08)),
                    step=0.5,
                    key="nuovo_ricarico_cliente_riv_v1035"
                )
            with col_riv4:
                st.markdown("<br>", unsafe_allow_html=True)
                if st.button("SALVA RICARICHI", key="v1035_salva_ricarichi_riv"):
                    ok_riv, msg_riv = v1035_salva_ricarichi_totale(
                        utente_riv_mod,
                        nuovo_ricarico_riv,
                        nuovo_ricarico_cliente
                    )
                    st.success(f"Ricarichi salvati per {utente_riv_mod}: acquisto {nuovo_ricarico_riv:.2f}% / cliente {nuovo_ricarico_cliente:.2f}%")
                    if msg_riv:
                        st.caption(msg_riv)
                    st.rerun()

            p1, p2, p3 = st.columns(3)
            p1.markdown(v1035_card("Costo SA-TEC esempio", euro(100)), unsafe_allow_html=True)
            p2.markdown(v1035_card("Prezzo Riv/Gross", euro(v1035_prezzo_riv(100, nuovo_ricarico_riv))), unsafe_allow_html=True)
            p3.markdown(v1035_card("Prezzo Cliente Finale", euro(v1035_prezzo_cliente(100, nuovo_ricarico_riv, nuovo_ricarico_cliente))), unsafe_allow_html=True)
...
st.caption("Versione V1035 - CRM blu e ricarico vendita Riv/Gross")
