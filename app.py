...
# =========================================================
# CONFIGURATORE PRO - INTERFACCIA MODERNA UI/UX (DARK & BLUE)
# =========================================================
...
st.markdown("""
<style>
    /* Sfondo generale e palette tech */
    .stApp { background-color: #0f172a; color: #f1f5f9; }
    
    /* Forzatura testi scuri nei moduli di input per leggibilità */
    ...
    /* La Nuova Ricevuta/Fattura Professionale */
    .invoice-box {
    ...
    /* Form e Contenitori interni Streamlit */
...
st.sidebar.markdown("### 🔐 AREA PARTNER")
if not st.session_state.auth:
    ...
    if st.sidebar.button("Accedi al Sistema"):
        if u_in in utenti_totali and utenti_totali[u_in]["password"] == p_in:
            ...
            st.rerun()
        else: st.sidebar.error("ID o Password non corretti.")
else:
    st.sidebar.markdown(f"""
    <div style="background: #1e293b; padding: 15px; border-radius: 8px; border-left: 4px solid #38bdf8; margin-bottom: 20px;">
        <span style="font-size:11px; color:#94a3b8; display:block;">Connesso come:</span>
        <strong style="color:#ffffff; font-size:15px;">{st.session_state.dati.get('azienda', st.session_state.user)}</strong>
        <span style="font-size:12px; color:#38bdf8; display:block; margin-top:2px;">Profilo: {st.session_state.profilo}</span>
    </div>
    """, unsafe_allow_html=True)
...
try: ricarico_corrente = float(st.session_state.dati.get("ricarico", "0")) if float(st.session_state.dati.get("ricarico", "0")) > 0 else ricarico_default(st.session_state.profilo)
except: ricarico_corrente = ricarico_default(st.session_state.profilo)

voci = ["📐 Calcolo & Configurazione", "📋 Registro Offerte"]
if st.session_state.profilo == "SA-TEC": voci.append("🛠️ CRM Gestione")
sezione = st.sidebar.radio("Navigazione", voci)

# =========================================================
# SEZIONE 1: CONFIGURATORE CORE
# =========================================================
if sezione == "📐 Calcolo & Configurazione":
    ...
    # Logica di Calcolo Economico Real-time
    ...
    <div class="kpi-container">
        ...
        <div class="kpi-card" style="border-top-color: #10b981;"><div class="kpi-title">Margine Applicato</div><div class="kpi-value">""" + str(ricarico_corrente) + """ %</div></div>
    ...
    with st.form("salva_offerta"):
        ...
        if st.form_submit_button("Conferma ed Emetti Offerta"):
            if not n_client and not a_client: st.error("Inserire un'intestazione valida per salvare il file.")
                ...
                st.success(f"🚀 Offerta commercializzata con successo! Codice assegnato: {nuovo_cod}. Controlla lo storico per visualizzarla.")

# =========================================================
# SEZIONE 2: REGISTRO E DOCUMENT GENERATOR (LA NUOVA SCHEDA)
# =========================================================
    ...
    if not offerte: st.info("Nessun preventivo presente in archivio.")
        ...
        # Rendering Grafico della Fattura/Preventivo ad impatto Visivo Elevato
        html_stampa = f"""
...
# =========================================================
# SEZIONE 3: CRM GESTIONE (SOLO ADMIN)
# =========================================================
    ...
    with st.form("crea_nuovo_partner"):
        ...
        if st.form_submit_button("Genera Account Privato"):
            if not r_az: st.error("Inserire la ragione sociale dell'azienda partner.")
            else:
                ...
                st.success(f"✨ Utente Configurato!\n* **ID LOGIN:** `{nuovo_id}`\n* **PASSWORD TEMPORANEA:** `{nuova_pwd}`\n* **RICARICO:** {ric_final}%")
